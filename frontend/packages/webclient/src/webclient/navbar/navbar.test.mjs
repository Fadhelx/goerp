...
assert.equal(findAll(settingsDropdown, (node) => String(node.className).includes("o_navbar_dropdown_header") && node.textContent === "Technical" && node.dataset?.menuLevel === "0").length, 1);
assert.equal(findAll(settingsDropdown, (node) => String(node.className).includes("o_navbar_submenu_toggle")).length, 0);
