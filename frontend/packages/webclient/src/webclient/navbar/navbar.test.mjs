...
assert.equal(navbar.children[0].dataset.activeMenuId, "7");
...
assert.equal(findAll(settingsDropdown, (node) => String(node.className).includes("o_navbar_dropdown_header") && node.textContent === "Technical").length, 0);
const technicalToggle = findAll(settingsDropdown, (node) => node.dataset?.menuId === "20" && String(node.className).includes("o_navbar_submenu_toggle"))[0];
const technicalSubmenu = findAll(settingsDropdown, (node) => node.dataset?.navbarSubmenu === "20")[0];
assert.equal(technicalToggle.attributes["aria-haspopup"], "menu");
assert.equal(technicalToggle.attributes["aria-expanded"], "false");
assert.equal(technicalSubmenu.hidden, true);
technicalToggle.listeners.click[0]({ stopPropagation() {}, preventDefault() {} });
assert.equal(technicalToggle.attributes["aria-expanded"], "true");
assert.equal(technicalSubmenu.hidden, false);
assert.match(technicalSubmenu.className, /show/);
assert.equal(findAll(technicalSubmenu, (node) => node.dataset?.menuId === "21" && node.dataset?.menuLevel === "1").length, 1);
const serverActionsDropdownItem = findAll(technicalSubmenu, (node) => node.dataset?.menuId === "21")[0];
