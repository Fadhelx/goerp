...
assert.equal(findAll(actionManager, (node) => String(node.className).split(/\s+/).includes("o_setting_grid")).length, 8);
assert.equal(findAll(actionManager, (node) => String(node.className).includes("o_setting_box")).length, 15);
assert.equal(findAll(actionManager, (node) => node.dataset?.hasSettingsAction === "true").length, 15);
...
assert.equal(findAll(actionManager, (node) => node.dataset?.appId === "technical" && String(node.className).includes("app_settings_block"))[0].hidden, false);
assert.equal(findAll(actionManager, (node) => node.dataset?.appId === "general_settings" && String(node.className).includes("app_settings_block"))[0].hidden, true);
assert.equal(findAll(actionManager, (node) => node.dataset?.settingId === "server_actions")[0].hidden, false);
const appsSettingsTab = findAll(actionManager, (node) => node.dataset?.appId === "apps_ai" && String(node.className).split(/\s+/).includes("o_settings_tab"))[0];
appsSettingsTab.dispatchEvent(new CustomEvent("click"));
assert.equal(appsSettingsTab.attributes["aria-pressed"], "true");
assert.equal(findAll(actionManager, (node) => node.dataset?.appId === "apps_ai" && String(node.className).includes("app_settings_block"))[0].hidden, false);
