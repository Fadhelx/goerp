...
var BaseMigrations = []Migration{
	...
	{Version: 74, Name: "workflow_node", SQL: `CREATE TABLE IF NOT EXISTS workflow_node (id BIGSERIAL PRIMARY KEY, name TEXT NOT NULL, code TEXT, workflow_id BIGINT NOT NULL, model TEXT, sequence INTEGER NOT NULL DEFAULT 10, active BOOLEAN NOT NULL DEFAULT true, type TEXT, responsible_group_ids TEXT, responsible_user_ids TEXT, responsible_python_code TEXT, responsible_condition TEXT, schedule_activity BOOLEAN NOT NULL DEFAULT false, schedule_activity_field_id BIGINT, schedule_activity_days INTEGER, state TEXT, button_type TEXT, button_name TEXT, allow_forward BOOLEAN NOT NULL DEFAULT false, escalation BOOLEAN NOT NULL DEFAULT false, escalation_delay_type TEXT, escalation_delay INTEGER, escalation_node_id BIGINT)`},
